...
void V0ToV1AdapterProcess::registered(
    ...
    const MasterInfo& masterInfo)
{
  LOG(INFO) << "Registered with the Mesos master; connecting the scheduler";
...
void V0ToV1AdapterProcess::disconnected()
{
  ...
  LOG(INFO) << "Disconnecting the scheduler";
...
void V0ToV1AdapterProcess::error(const string& message)
{
  ...
  // There might be an error during the implicit registration happening on
  // driver initialization. Since `Scheduler.connect` is called upon successful
  // registration only, the scheduler will never try to subscribe and hence
  // will never receive the error. The logic for the workaround is that if
  // we have received an error from the driver, connection to the master has
  // been successful, hence it is reasonable to connect the scheduler and
  // send the error in responce to the subscribe call.
  if (!subscribeCall) {
