    ...
    const hashset<ContainerID>& orphans)
{
  ...
  foreach (const ContainerState& run, recovered) {
    ...
    if (containerLaunchInfo.isSome()) {
      containers_[containerId]->launchEnvironment =
          containerLaunchInfo->environment();

      containers_[containerId]->workingDirectory =
          containerLaunchInfo->working_directory();
    }
  }
    ...
    bool checkpoint)
{
  ...
  if (container->config.has_container_class() &&
    ...
    CHECK(containerId.has_parent());
    if (containers_[containerId.parent()]->launchEnvironment.isSome()) {
      containerEnvironment.CopyFrom(
          containers_[containerId.parent()]->launchEnvironment.get());
    }
  }
  ...
  // This is implicitly checkpointed as part of `launchInfo`.
  container->launchEnvironment = containerEnvironment;
