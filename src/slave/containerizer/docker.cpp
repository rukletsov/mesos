    ...
    const list<Docker::Container>& _containers)
{
  ...
  foreach (const Docker::Container& container, _containers) {
    ...
    if (!containers_.contains(id.get())) {
      // TODO(alexr): After the deprecation cycle (started in 0.29) we should
      // set the timeout to 0. Graceful shutdown happens in the agent and / or
      // the executor, while the containerizer kills the container instantly.
      futures.push_back(
          docker->stop(
              container.id,
