...
{
  return process::defaultClone([taskPid, namespaces, func]() -> int {
    if (taskPid.isSome()) {
      foreach (const string& ns, namespaces) {
        Try<Nothing> setns = ns::setns(taskPid.get(), ns);
        if (setns.isError()) {
          // Abort continues to perform the health check.
          LOG(FATAL) << "Failed to enter the " << ns << " namespace of "
                     << "task (pid: '" << taskPid.get() << "'): "
                     << setns.error();
        } else {
          VLOG(1) << "Enter the " << ns << " namespace of "
...
{
#ifdef __linux__
  clone = lambda::bind(&setNsClone, lambda::_1, taskPid, namespaces);
#endif
}
...
Future<Nothing> HealthCheckerProcess::_tcpHealthCheck()
{
  ...
  // TODO(haosdent): Need to replace `bash` with a tiny binary to support
  ...
  if (s.isError()) {
    return Failure("Failed to exec the bash subprocess: " + s.error());
  }
  ...
  return await(
    ...
    .after(timeout,
      ...
      if (bashPid != -1) {
        ...
        VLOG(1) << "Killing the processs of TCP health check "
                << bashPid;

        os::killtree(bashPid, SIGKILL);
      }

      return Failure("bash has not returned after " + stringify(timeout));
    })
    ...
    const tuple<Future<Option<int>>, Future<string>, Future<string>>& t)
{
  ...
  if (!status.isReady()) {
    return Failure(
        "Failed to get the exit status of the bash subprocess: " +
        (status.isFailed() ? status.failure() : "discarded"));
  }
