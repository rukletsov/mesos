...
private:
  ...
  struct Metrics
  {
    Metrics(const BalloonSchedulerProcess& _scheduler)
      : uptime_secs(
            FRAMEWORK_METRICS_PREFIX + "/uptime_secs",
            defer(_scheduler, &BalloonSchedulerProcess::_uptime_secs)),
        registered(
            FRAMEWORK_METRICS_PREFIX + "/registered",
            defer(_scheduler, &BalloonSchedulerProcess::_registered)),
        tasks_finished(FRAMEWORK_METRICS_PREFIX + "/tasks_finished"),
        tasks_oomed(FRAMEWORK_METRICS_PREFIX + "/tasks_oomed"),
        launch_failures(FRAMEWORK_METRICS_PREFIX + "/launch_failures"),
        abnormal_terminations(
            FRAMEWORK_METRICS_PREFIX + "/abnormal_terminations")
    {
...
int main(int argc, char** argv)
{
  ...
  if (flags.executor_command.isSome()) {
    ...
    command = path::join(
        os::realpath(Path(argv[0]).dirname()).get(), EXECUTOR_BINARY);
  }
