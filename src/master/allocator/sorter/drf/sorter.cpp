...
double DRFSorter::calculateShare(const string& name)
{
  ...
  // TODO(mpark): Since we do not yet account for non-scalar
  // resources, we can safely aggregate the resources and pull out
  // the scalar resources as before.
  Resources _resources = Resources::sum(resources.values());
  Resources _allocations = Resources::sum(allocations[name].values());
  ...
  foreach (const string& scalar, scalars) {
    Option<Value::Scalar> total = _resources.get<Value::Scalar>(scalar);

    if (total.isSome() && total.get().value() > 0) {
      Option<Value::Scalar> allocation =
        _allocations.get<Value::Scalar>(scalar);
