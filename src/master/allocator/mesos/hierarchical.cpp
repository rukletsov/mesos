  ...
  foreach (const SlaveID& slaveId, slaveIds) {
    ...
    foreach (const string& role, roleSorter->sort()) {
      foreach (const string& frameworkId_,
        ...
        // To avoid allocating non-revocable resources beyond quota, mark all
        // resources that are to be offered for a framework from the quota'ed
        // role as revocable. If the framework has not opted for revocable
        // resources, no offers will be made beyond quota.
        //
        // NOTE: Due to the coarse-grained nature of allocations, some
        // resources beyond quota may still be allocated as non-revocable.
        //
        // NOTE: Revocable resources do not count towards the role's quota.
        Resources offerableResources = resources;
        if (quotas.contains(role)) {
          foreach (Resource& resource, offerableResources) {
        ...
        VLOG(2) << "Allocating " << offerableResources << " on slave "
        ...
        // NOTE: We may allocate resources as revocable, but we still track
        // them as non-revocable to ensure the resource math is correct.
        offerable[frameworkId][slaveId] += offerableResources;
        ...
        // roles.
        frameworkSorters[role]->add(slaveId, offerableResources);
        frameworkSorters[role]->allocated(
            frameworkId_, slaveId, offerableResources);

        roleSorter->allocated(role, slaveId, offerableResources.unreserved());

        if (quotas.contains(role)) {
          quotaRoleSorter->allocated(
              role, slaveId, offerableResources.unreserved().nonRevocable());
        }
      }
    }
  }
