    ...
    const hashset<SlaveID>& slaveIds_)
{
  ...
  foreach (const SlaveID& slaveId, slaveIds) {
    foreach (const string& role, quotaRoleSorter->sort()) {
      ...
      foreach (const string& frameworkId_, frameworkSorters[role]->sort()) {
        ...
        // accepted by some framework during the second allocation stage.
  ...
  //     `remainingClusterResources` - `allocatedInTheSecondStage` is empty.
  ...
  //     (`allocatedInTheSecondStage` + potential offer). In this case we
  //     skip this agent and continue to the next one.
  Resources allocatedInTheSecondStage;
  ...
  foreach (const SlaveID& slaveId, slaveIds) {
    ...
    if ((remainingClusterResources - allocatedInTheSecondStage).empty()) {
      break;
    }
    ...
    foreach (const string& role, roleSorter->sort()) {
      foreach (const string& frameworkId_,
        ...
        if (!remainingClusterResources.contains(
                 allocatedInTheSecondStage + resources)) {
          continue;
        }
