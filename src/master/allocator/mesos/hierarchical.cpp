    ...
    const hashmap<string, Quota>& quotas)
{
  ...
  // Though it is not strictly necessary for the allocator to function
  // correctly, skip recovery if there are no expected agents.
  ...
  LOG(INFO) << "Triggered allocator recovery: waiting for "
            << expectedAgentCount << " slaves to reconnect or "
    ...
    const hashset<SlaveID>& slaveIds_)
{
  ...
  foreach (const SlaveID& slaveId, slaveIds) {
    foreach (const string& role, quotaRoleSorter->sort()) {
      ...
      foreach (const string& frameworkId_, frameworkSorters[role]->sort()) {
        ...
        // If the framework has suppressed offers, ignore. Unallocated part
        // of the quota will not be allocated to other roles.
        ...
        // If the framework filters these resources, ignore. Unallocated
  ...
  foreach (const SlaveID& slaveId, slaveIds) {
    Resources resources = slaves[slaveId].total - slaves[slaveId].allocated;
    remainingClusterResources += resources;
  }
  ...
  //     (`allocatedForWDRF` + potential offer).
  Resources allocatedForWDRF;

  // At this point resources for quotas are allocated. Proceed with
  // allocating the remaining free pool using WDRF.
  foreach (const SlaveID& slaveId, slaveIds) {
