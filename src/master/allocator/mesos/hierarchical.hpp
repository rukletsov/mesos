...
protected:
  ...
  // NOTE: We currently tie quota to roles, but this may change in the future.
  struct Role
  {
    mesos::master::RoleInfo info;

    // Setting quota for a role changes the order the role's frameworks are
    // offered resources. Quota goes before fair share, hence setting quota
    // moves the role's frameworks towards the allocation queue's front.
