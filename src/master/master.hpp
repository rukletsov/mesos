...
private:
  ...
  protected:
    ...
    // reasonabily be satisfied given the current cluster capacity. The
    ...
    // it can happen that there are not enough free resources at allocator's
    // disposal when it is notified about the quota request, but at this
    // point it's too late to rescind.
    //
    // While rescinding, we adhere to the following rules:
    //   * Rescind at least as much resources as there are in quota request.
    //   * Rescind all offers from an agent in order to make makes the
    //     potential offer bigger, which increases chances a quota'ed
    //     framework will be able to use the offer.
    //   * Rescind offers from at least `numF` agents to make it possible
    //     (but not necessarily happening, remember fair shares) that each
    //     framework in the role for which quota is set gets an offer
    //     (`numF` is the number of frameworks in the quota'ed role).
    //     Though this is not strictly necessary, we think this will
    //     increase the debugability and will improve user experience.
    //
