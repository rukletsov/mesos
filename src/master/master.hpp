...
private:
  ...
  public:
    ...
    {
      // TODO(joerg84): Remove once later code uses master*.
  ...
  protected:
    // Validates whether a quota request can be satisfied at the moment. We
    // cannot give a precise answer here, it is a scientific guess, because:
    ...
    //     request;
    //   - even if there are enough resources at the moment of this validation,
    ...
    // may update the way we validate satisfiability in the master.
    Option<Error> validateSatisfiability(
  ...
  private:
    ...
    Master* master;
    // NOTE: The quota specific pieces of the Admin API are factored
  ...
  // A list of granted quotas in the cluster. Role is used as a key because
  // currently quotas are set per role.
