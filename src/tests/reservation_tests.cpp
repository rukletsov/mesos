...
// This tests that a framework can't unreserve resources that weren't
// part of the offer.
TEST_F(ReservationTest, DropUnreserveWithInvalidRole)
{
  FrameworkInfo frameworkInfo = DEFAULT_FRAMEWORK_INFO;
  frameworkInfo.set_role("role");
  ...
  // Expect that the unreserve offer operation will be dropped.
  EXPECT_CALL(allocator, updateAllocation(_, _, _, _))
    .Times(0);
  ...
  // Attempt to unreserve resources using a different role than the one the
  // framework is registered with.
  Resources dynamicallyReservedInvalidRole = unreserved.flatten(
      "yoyo",
      createReservationInfo(frameworkInfo.principal())).get();
  ...
  // Unreserve the resources.
  driver.acceptOffers(
      {offer.id()}, {UNRESERVE(dynamicallyReservedInvalidRole)}, filters);
