...
// This command stalls every other invocation.
// For all runs i in Nat0, the following case i % 2 applies:
//
// Case 0:
//   - Attempt to print the nonexistent temporary file.
//   - Create the temporary file.
//   - Exit with a non-zero status.
//
// Case 1:
...
#define STALLING_CHECK_COMMAND(path)                                    \
  "(cat " + path + " && " + SLEEP_COMMAND(1000) +                       \
  ") || (touch " + path + "; exit 1)"
#else
#define STALLING_CHECK_COMMAND(path)                                    \
  "powershell -command "                                                \
  "$gi_err = Get-Item -ErrorAction SilentlyContinue \"" + path + "\";"  \
  "if (-not $?) {"                                                      \
  "  set-content -Path (\"" + path + "\") -Value ($null);"              \
  "  exit 1"                                                            \
  "} else {" +                                                          \
...
TEST_F(CommandExecutorCheckTest, CommandCheckStatusChange)
{
  ...
  const string checkCommand = FLAPPING_CHECK_COMMAND(tmpPath);

  checkInfo->mutable_command()->mutable_command()->set_value(checkCommand);
...
TEST_F(CommandExecutorCheckTest, CommandCheckTimeout)
{
  ...
  const string tmpPath = path::join(os::getcwd(), "XXXXXX");
  const string checkCommand = STALLING_CHECK_COMMAND(tmpPath);
...
TEST_F(CommandExecutorCheckTest, HTTPCheckDelivered)
{
  Try<Owned<cluster::Master>> master = StartMaster();
  ASSERT_SOME(master);
...
TEST_F(CheckTest, DefaultExecutorHTTPCheckDelivered)
{
  ...
  AWAIT_READY(updateTaskRunning);
  const auto& taskRunning = updateTaskRunning->status();
  ...
  callAcknowledge(&mesos, frameworkId, taskRunning);
