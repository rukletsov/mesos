...
// status is reflected in the status updates sent as reconcilliation
...
// containerizer and verifies that the healthy status is reflected in
// the status updates sent as reconcilliation answers, and in the state
// endpoint of both the master and the agent.
...
// verifies that the healthy status is reflected in the status updates
// sent as reconcilliation answers, and in the state endpoint of both
...
// sent to the framework and reflected in the state endpoint of both
// the master and the agent.
...
// out, and verifies that the health check is retried after the
// timeout.
...
TEST_F(HealthCheckTest, HealthyTaskViaHTTP)
{
  ...
  // To avoid external program dependencies, use the `/health` endpoint
  // of the agent as HTTP health check target here.
  HealthCheck healthCheck;
  healthCheck.set_type(HealthCheck::HTTP);
  healthCheck.mutable_http()->set_port(agent.get()->pid.address.port);
  healthCheck.mutable_http()->set_path("/" + agent.get()->pid.id + "/health");
  healthCheck.set_delay_seconds(0);
  healthCheck.set_interval_seconds(0);
  healthCheck.set_grace_period_seconds(0);
...
TEST_F(HealthCheckTest, HealthyTaskViaTCP)
{
  ...
  // To avoid external program dependencies, use the port of the agent
  // as TCP health check target here.
  HealthCheck healthCheck;
  healthCheck.set_type(HealthCheck::TCP);
  healthCheck.mutable_tcp()->set_port(agent.get()->pid.address.port);
  healthCheck.set_delay_seconds(0);
  healthCheck.set_interval_seconds(0);
  healthCheck.set_grace_period_seconds(0);
...
// containerizer.
TEST_F(HealthCheckTest, ROOT_HealthyTaskViaHTTPWithContainerImage)
{
  master::Flags masterFlags = CreateMasterFlags();
  masterFlags.allocation_interval = Milliseconds(50);
  Try<Owned<cluster::Master>> master = StartMaster(masterFlags);
  ...
  // To avoid external program dependencies, use the `/health` endpoint
  // of the agent as HTTP health check target here. Because mesos
  // containerizer uses host network in this test case, it would
  // success when connect to the agent.
  HealthCheck healthCheck;
  healthCheck.set_type(HealthCheck::HTTP);
  healthCheck.mutable_http()->set_port(agent.get()->pid.address.port);
  healthCheck.mutable_http()->set_path("/" + agent.get()->pid.id + "/health");
...
// containerizer.
TEST_F(HealthCheckTest, ROOT_HealthyTaskViaTCPWithContainerImage)
{
  master::Flags masterFlags = CreateMasterFlags();
  masterFlags.allocation_interval = Milliseconds(50);
  Try<Owned<cluster::Master>> master = StartMaster(masterFlags);
  ...
  // To avoid external program dependencies, use the port of the agent
  // as TCP health check target here. Because mesos containerizer uses
  // host network in this test case, it would success when connect to
  // the agent.
  HealthCheck healthCheck;
  healthCheck.set_type(HealthCheck::TCP);
  healthCheck.mutable_tcp()->set_port(agent.get()->pid.address.port);
...
TEST_F(HealthCheckTest, ROOT_DOCKER_DockerHealthyTaskViaHTTP)
{
  ...
  // Use Netcat here because could not connect to the endpoints of
  // agent and master when docker container uses bridge network mode.
  ...
  // launch Netcat to serve the request.
  HealthCheck healthCheck;
  healthCheck.set_type(HealthCheck::HTTP);
  healthCheck.mutable_http()->set_port(8000);
  healthCheck.set_delay_seconds(0);
  healthCheck.set_interval_seconds(0);
...
TEST_F(HealthCheckTest, ROOT_DOCKER_DockerHealthyTaskViaTCP)
{
  ...
  // Use Netcat here because could not connect to the endpoints of
  // agent and master when docker container uses bridge network mode.
  TaskInfo task = createTask(offers.get()[0], "nc -lk -p 8000 -e echo");
  ...
  HealthCheck healthCheck;
  healthCheck.set_type(HealthCheck::TCP);
  healthCheck.mutable_tcp()->set_port(8000);
  healthCheck.set_delay_seconds(0);
  healthCheck.set_interval_seconds(0);
  healthCheck.set_grace_period_seconds(15);
