...
protected:
  const hashmap<FrameworkID, Resources> EMPTY_ALLOCATION;
...
TEST_F(HierarchicalAllocatorTest, UnreservedDRF)
{
  ...
  allocator->addSlave(
      slave1.id(), slave1, None(), slave1.resources(), EMPTY_ALLOCATION);
  ...
  allocator->addSlave(
      slave2.id(), slave2, None(), slave2.resources(), EMPTY_ALLOCATION);
  ...
  allocator->addSlave(
      slave3.id(), slave3, None(), slave3.resources(), EMPTY_ALLOCATION);
  ...
  allocator->addSlave(
      slave4.id(), slave4, None(), slave4.resources(), EMPTY_ALLOCATION);
  ...
  allocator->addSlave(
      slave5.id(), slave5, None(), slave5.resources(), EMPTY_ALLOCATION);
...
TEST_F(HierarchicalAllocatorTest, MaintenanceInverseOffers)
{
  ...
  allocator->addSlave(
      agent.id(), agent, None(), agent.resources(), EMPTY_ALLOCATION);
...
TEST_F(HierarchicalAllocatorTest, SameShareFairness)
{
  ...
  allocator->addSlave(
      slave.id(), slave, None(), slave.resources(), EMPTY_ALLOCATION);
...
TEST_F(HierarchicalAllocatorTest, QuotaProvidesQuarantee)
{
  ...
  allocator->addSlave(
      agent1.id(), agent1, None(), agent1.resources(), EMPTY_ALLOCATION);
  ...
  allocator->addSlave(
      agent2.id(), agent2, None(), agent2.resources(), EMPTY_ALLOCATION);
...
TEST_F(HierarchicalAllocatorTest, MultipleFrameworksInRoleWithQuota)
{
  ...
  allocator->addSlave(
      agent3.id(), agent3, None(), agent3.resources(), EMPTY_ALLOCATION);
...
TEST_F(HierarchicalAllocatorTest, QuotaAllocationGranularity)
{
  ...
  EXPECT_EQ(framework1.id(), allocation.get().frameworkId);
  EXPECT_EQ(agent1.resources(), Resources::sum(allocation.get().resources));
  EXPECT_TRUE(Resources(agent1.resources()).contains(quota.info.guarantee()));
