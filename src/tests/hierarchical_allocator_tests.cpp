...
// Hierarchical DRF allocator (i.e. the way quota is satisfied) are in
// this file.
...
//     does not get new non-revocable offers (retroactivity).
...
// quota'ed role declines offers, some resources are laid aside for the
// role, so that a greedy framework from a non-quota'ed role cannot eat
// up all free resources.
TEST_F(HierarchicalAllocatorTest, QuotaProvidesQuarantee)
{
  ...
  const std::string ROLE_WITH_QUOTA{"role-with-quota"};
  const std::string ROLE_NO_QUOTA{"role-no-quota"};

  hashmap<FrameworkID, Resources> EMPTY;

  initialize(vector<string>{ROLE_WITH_QUOTA, ROLE_NO_QUOTA});

  // Create `framework1` and set quota for its role.
  FrameworkInfo framework1 = createFrameworkInfo(ROLE_WITH_QUOTA);
  allocator->addFramework(
      framework1.id(), framework1, hashmap<SlaveID, Resources>());
  ...
  FrameworkInfo framework2 = createFrameworkInfo(ROLE_NO_QUOTA);
  allocator->addFramework(
      framework2.id(), framework2, hashmap<SlaveID, Resources>());
  ...
  // ROLE_WITH_QUOTA share = 1 (cpus=1, mem=512) [quota: cpus=2, mem=1024]
  //   framework1 share = 1
  ...
  // `framework1` will be again offered all of `agent2`'s resources
  ...
  // Total cluster resources: cpus=1, mem=512.
  // ROLE_WITH_QUOTA share = 0.5 (cpus=1, mem=512) [quota: cpus=2, mem=1024]
  //   framework1 share = 1
  // ROLE_NO_QUOTA   share = 0
  //   framework2 share = 0
  ...
  // and only allocation should be declined resources to the quota'ed role.
...
TEST_F(HierarchicalAllocatorTest, RemoveQuota)
{
  ...
  allocator->removeQuota(ROLE_WITH_QUOTA);
  ...
  //   framework1 share = 1
  // ROLE_NO_QUOTA   share = 0.5 (cpus=1, mem=512)
  //   framework2 share = 1
}
...
TEST_F(HierarchicalAllocatorTest, MultipleFrameworksInRoleWithQuota)
{
  ...
  FrameworkInfo framework1a = createFrameworkInfo(ROLE_WITH_QUOTA);
  allocator->addFramework(
      framework1a.id(), framework1a, hashmap<SlaveID, Resources>());

  QuotaInfo quota1 = createQuotaInfo(ROLE_WITH_QUOTA, "cpus:4;mem:2048");
  allocator->setQuota(ROLE_WITH_QUOTA, quota1);
  ...
  FrameworkInfo framework1b = createFrameworkInfo(ROLE_WITH_QUOTA);
  allocator->addFramework(
  ...
  // ROLE_WITH_QUOTA share = 1 (cpus=3, mem=1536) [quota: cpus=4, mem=2048]
  ...
  // its share is less than `framework1b`'s and `ROLE_WITH_QUOTA` still
  ...
  // ROLE_WITH_QUOTA share = 1 (cpus=4, mem=2048) [quota: cpus=4, mem=2048]
...
// to satisfy quota are no exceptions. A role may get more resources as
...
TEST_F(HierarchicalAllocatorTest, QuotaAllocationGranularity)
{
  ...
  QuotaInfo quota1 = createQuotaInfo(ROLE_WITH_QUOTA, "cpus:0.5;mem:200");
  allocator->setQuota(ROLE_WITH_QUOTA, quota1);
  ...
  // Total cluster resources: cpus=1, mem=512.
  // ROLE_WITH_QUOTA share = 1 (cpus=1, mem=512) [quota: cpus=0.5, mem=200]
  //   framework1 share = 1
  // ROLE_NO_QUOTA   share = 0
  //   framework2 share = 0
}
...
TEST_F(HierarchicalAllocatorTest, DRFWithQuota)
{
  ...
  // ROLE_WITH_QUOTA share = 0.25 (cpus=0.25, mem=128) [quota: cpus=0.25, mem=128] // NOLINT
  //   framework1 share = 1
  // ROLE_NO_QUOTA   share = 0
  //   framework2 share = 0
  ...
  QuotaInfo quota1 = createQuotaInfo(ROLE_WITH_QUOTA, "cpus:0.25;mem:128");
  ...
  // ROLE_WITH_QUOTA share = 0.25 (cpus=0.25, mem=128) [quota: cpus=0.25, mem=128] // NOLINT
  //   framework1 share = 1
  // ROLE_NO_QUOTA   share = 0.75 (cpus=0.75, mem=384)
  //   framework2 share = 0
...
// accept more resources if getting them offered. However, if there are not
...
TEST_F(HierarchicalAllocatorTest, QuotaAgainstStarvation)
{
  ...
  // We set quota for the "starving" `ROLE_WITH_QUOTA` role.
  QuotaInfo quota1 = createQuotaInfo(ROLE_WITH_QUOTA, "cpus:2;mem:1024");
  allocator->setQuota(ROLE_WITH_QUOTA, quota1);
