...
TEST_F(SlaveTest, CommandTaskWithKillPolicy)
{
  ...
  // observe TASK_KILLED updated sooner than after `gracePeriod`
...
TEST_F(SlaveTest, ExecutorShutdownGracePeriod)
{
  ...
  Future<Nothing> executorShutdownTimedOut =
  ...
  EXPECT_TRUE(executorShutdownTimedOut.isPending());
  ...
  AWAIT_READY(executorShutdownTimedOut);
  ...
  AWAIT_READY(lostExecutorId);
  EXPECT_EQ(DEFAULT_EXECUTOR_ID, lostExecutorId.get());
