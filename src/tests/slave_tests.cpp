...
TEST_F(SlaveTest, CustomExecutorGraceShutdownPeriod)
{
  // Pausing the clock is not necessary, but ensures that batch allocation
  // does not kick in unpredictably.
  Clock::pause();
  ...
  // NOTE: We don't use `StartSlave()` because we need to access these flags.
  slave::Flags agentFlags = CreateSlaveFlags();
  Try<PID<Slave>> agent = StartSlave(&containerizer, agentFlags);
  ASSERT_SOME(agent);
  ...
  // disabled in tests, hence we do not observe crashes induced by this test.
  {
    ...
    EXPECT_CALL(containerizer, destroy(_))
      .WillOnce(DoAll(FutureSatisfy(&containerDestroyed),
                InvokeDestroy(&containerizer)))
    ...
    post(master.get(), agent.get(), shutdownMessage);
