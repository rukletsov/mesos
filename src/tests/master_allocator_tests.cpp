...
{
  TestAllocator allocator = this->CreateAllocator();
...
TYPED_TEST(MasterAllocatorTest, FrameworkReregistersFirst)
{
  ...
  // We want to ensure all object associated with the current master
  // (e.g. allocator) are destroyed once the master is shut down.
  {
    TestAllocator allocator = this->CreateAllocator();
