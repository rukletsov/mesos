...
{
public:
  MasterAllocatorTest() : testAllocator(NULL) {}

  // We favour a reference here for self-documentation. The function
  // never returns null. Returning a reference brings this up to the
  // interface level.
  TestAllocator& allocator() const
  {
    return *CHECK_NOTNULL(testAllocator);
  }
...
protected:
  virtual void SetUp()
  {
    ...
    testAllocator = new TestAllocator(createAllocator());
  }
  ...
  process::Owned<Allocator> createAllocator()
  {
...
TYPED_TEST(MasterAllocatorTest, Whitelist)
{
  ...
  EXPECT_CALL(this->allocator(),
              updateWhitelist(Option<hashset<string>>(hosts)))
...
TYPED_TEST(MasterAllocatorTest, FrameworkReregistersFirst)
{
  ...
  TestAllocator allocator2(this->createAllocator());
