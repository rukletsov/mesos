...
using mesos::internal::master::allocation::Allocator;
using mesos::internal::master::allocation::HierarchicalDRFAllocator;
...
{
protected:
  MasterAllocatorTest() : allocator_(new MockAllocator<T>()) {}

  // Several tests have been reported flaky if allocation is not
  // ceased explicitly.
  void StopAllocator()
  {
    allocator_.reset();
  ...
  MockAllocator<T>* allocator() const
  {
    CHECK_NOTNULL(allocator_.get());
    return allocator_.get();
  }

  process::Owned<MockAllocator<T>> allocator_;
};
