...
TEST(SorterTest, SplitResourceShares)
{
  ...
  sorter.allocated("a", slaveId, Resources::parse("cpus:9;mem:9;disk:9").get());
  sorter.allocated(
...
TEST(SorterTest, NonScalarResourcesAcrossSlaves)
{
  DRFSorter sorter;
  ...
  EXPECT_EQ(allocation[slaveA], slaveResources);
  EXPECT_EQ(allocation[slaveB], slaveResources);
}
...
TEST(SorterTest, UpdateNonScalarResourcesAcrossSlaves)
{
  DRFSorter sorter;
  ...
  // Construct an offer operation.
  Resources reserved = slaveResources.flatten("role");

  // Update the resources for the client.
  sorter.update("framework", slaveA, slaveResources, reserved);
  sorter.update("framework", slaveB, slaveResources, reserved);

  hashmap<SlaveID, Resources> allocation = sorter.allocation("framework");
  EXPECT_EQ(allocation[slaveA], reserved);
  EXPECT_EQ(allocation[slaveB], reserved);
}
