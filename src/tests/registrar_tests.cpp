...
TEST_P(RegistrarTest, UpdateQuota)
{
  ...
  Resources quotaResources = Resources::parse("cpus:1;mem:1024").get();
  Resources quotaResourcesAlt = Resources::parse("cpus:2").get();

  QuotaInfo quota1;
  quota1.set_role(role1);
  quota1.mutable_guarantee()->CopyFrom(
      static_cast<const RepeatedPtrField<Resource>&>(
          quotaResources.flatten(role1)));

  QuotaInfo quota2;
  quota2.set_role(role2);
  quota2.mutable_guarantee()->CopyFrom(
      static_cast<const RepeatedPtrField<Resource>&>(
          quotaResources.flatten(role2)));
...

  {
    ...
    Resources storedResources(registry.get().quotas(0).info().guarantee(0));
    storedResources += registry.get().quotas(0).info().guarantee(1);
    EXPECT_EQ(quotaResources, storedResources.flatten());

    // Change quota for `role1`.
    quota1.mutable_guarantee()->CopyFrom(
        static_cast<const RepeatedPtrField<Resource>&>(
            quotaResourcesAlt.flatten(role1)));
...

  {
    ...
    Resources storedResources(registry.get().quotas(0).info().guarantee(0));
    EXPECT_EQ(quotaResourcesAlt, storedResources.flatten());
...

  {
    ...
    // Check that the recovered quotas match those we stored previously. Note
    // that we assume quota messages are stored in order they have been added.
    ...
    Resources storedResources(registry.get().quotas(1).info().guarantee(0));
    storedResources += registry.get().quotas(1).info().guarantee(1);
    EXPECT_EQ(quotaResources, storedResources.flatten());

    // Change quota for `role2`.
    quota2.mutable_guarantee()->CopyFrom(
        static_cast<const RepeatedPtrField<Resource>&>(
            quotaResourcesAlt.flatten(role2)));
...

  {
    ...
    // previously. Note that we assume quota messages are stored in order they
    // have been added and update does not change the order.
    ...
    Resources storedResources1(registry.get().quotas(0).info().guarantee(0));
    EXPECT_EQ(quotaResourcesAlt, storedResources1.flatten());

    EXPECT_EQ(role2, registry.get().quotas(1).info().role());
    ASSERT_EQ(1, registry.get().quotas(1).info().guarantee().size());

    Resources storedResources2(registry.get().quotas(1).info().guarantee(0));
    EXPECT_EQ(quotaResourcesAlt, storedResources2.flatten());
  }
}
...
TEST_P(RegistrarTest, RemoveQuota)
{
...

  {
    ...
    quota1.mutable_guarantee()->CopyFrom(
        static_cast<const RepeatedPtrField<Resource>&>(
            quotaResources1.flatten(role1)));

    QuotaInfo quota2;
    quota2.set_role(role2);
    quota2.mutable_guarantee()->CopyFrom(
        static_cast<const RepeatedPtrField<Resource>&>(
            quotaResources2.flatten(role2)));
