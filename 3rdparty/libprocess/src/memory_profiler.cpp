...
MemoryProfiler::DiskArtifact::DiskArtifact(const std::string& _filename)
  : filename(_filename),
    _timestamp(Error("Not yet generated."))
{}


const Try<time_t>& MemoryProfiler::DiskArtifact::timestamp() const
{
  return _timestamp;
}
    ...
    std::function<Try<Nothing>(const std::string&)> generator)
{
  ...
  if (_timestamp.isSome() && _timestamp.get() == requestedTimestamp) {
    return Nothing();
  }

  Try<std::string> _path = path();
  if (_path.isError()) {
    return Error("Could not determine target path: " + _path.get());
  }

  Try<Nothing> result = generator(_path.get());
  ...
  if (result.isError()) {
    ...
    _timestamp = Error(result.error());
    return result;
  }

  _timestamp = requestedTimestamp;
...
// TODO(bevers): Add query parameter to configure the sampling interval.
  ...
  const Option<http::authentication::Principal>& principal)
{
  ...
  std::string message;
  if (wasActive.get()) {
    message += "Heap profiling is already active.<br/>";
  } else {
    message += "Started new heap profiling run.<br/>";
    profilingTimer = Clock::timer(
        duration,
  ...
  std::string remainingSeconds = stringify(static_cast<int>(
      profilingTimer->timeout().remaining().secs() + 0.5));

  message +=
    "You will be redirected to the result in" + remainingSeconds + "s.";

  const std::string redirectTag = "<meta http-equiv=\"refresh\" "
    "content=\"" + remainingSeconds + ";url=./stop\" />";

  return htmlResponse(redirectTag, message);
}
    ...
    const Option<http::authentication::Principal>&)
{
  ...
  std::string idString = stringify(generated.get());

  auto hyperlink = [](const std::string& text, const std::string& target) {
    return "<a href=\"" + target + "\">" + text + "</a>";
  };

  std::string downloadMessage = strings::format(
      "Profile #%s successfully created!<br/>"
      ...
      "(Note: These can take several minutes to generate!)<br/>",
      idString,
...
Try<time_t> MemoryProfiler::stopAndGenerateRawProfile()
{
  ...
  Try<bool> result = jemalloc.stopProfiling();
  ...
  if (result.isError()) {
    ...
    LOG(INFO) << "Not dumping profile; heap profiling was not active.";

    // Return the result of the last dump in this case.
    return jemallocRawProfile.timestamp();
  }
  ...
  // This is actually unique because the minimum sample duration is 1 second.
  time_t nextId = std::chrono::system_clock::to_time_t(
    ...
    const Option<http::authentication::Principal>&)
{
  ...
  if (jemallocRawProfile.timestamp().isError()) {
    return http::BadRequest(
        "No heap profile exists: " + jemallocRawProfile.timestamp().error());
  }

  if (requestedId.isSome() &&
      (requestedId.get() != jemallocRawProfile.timestamp().get())) {
    return http::BadRequest("Cannot serve requested id.");
  }
    ...
    const Option<http::authentication::Principal>&)
{
  ...
  if (jemallocRawProfile.timestamp().isError()) {
    return http::BadRequest(
        "No source profile exists: " + jemallocRawProfile.timestamp().error());
  }

  time_t rawId = jemallocRawProfile.timestamp().get();
  ...
  Try<Nothing> result = jeprofGraph.generate(
      rawId,
      [&](const std::string& outputPath) -> Try<Nothing> {
        if (!(requestedId.get() == jemallocRawProfile.timestamp().get())) {
  ...
  const Option<http::authentication::Principal>&)
{
...

  {
...

    {
      ...
      // as an option string. We should also display this here.
