...
MemoryProfiler::ProfilingRun::ProfilingRun(
  ...
  : id(id),
    timer(Clock::timer(duration, [this] {
      this->profiler.stopAndGenerateRawProfile();
    })),
    profiler(profiler)
{}


void MemoryProfiler::ProfilingRun::extend(const Duration& duration)
{
  Duration remaining = timer.timeout().remaining();
  Clock::cancel(timer);
  timer = Clock::timer(remaining + duration, [this] {
    this->profiler.stopAndGenerateRawProfile();
  });
}
  ...
  const Option<http::authentication::Principal>& principal)
{
  ...
  if (!currentRun.isSome()) {
    response.values["message"] = "Heap profiling was started externally.";
  ...
  message +=
    ...
    " Visit `/stop` to end the run prematurely.";
...
Try<time_t> MemoryProfiler::stopAndGenerateRawProfile()
{
  ...
  // Return the id of the last successful run i
  if (!currentRun.isSome()) {
